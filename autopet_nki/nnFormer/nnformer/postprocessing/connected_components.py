...
def load_remove_save(input_file: str, output_file: str, for_which_classes: list,
    ...
    #print(input_file[0])
    #print(output_file[0])
    img_in = sitk.ReadImage(input_file[0])

    #print("initial sitk image")
    #print(img_in.GetDirection())
    #print(img_in.GetSize())
    img_npy = sitk.GetArrayFromImage(img_in)
    #print("shape initial np array")
    #print(img_npy.shape)
    #img_npy_img = sitk.GetImageFromArray(img_npy)
    #print("Reverse")
    #print(img_npy_img.GetDirection())
    #print(img_npy_img.GetSize())

    #img_npy_in = np.squeeze(img_npy, axis=0)
    #print("shape_Squeeze")
    #print(img_npy_in.shape)
    """
    mask_nii = nib.load(input_file[0])
    mask = mask_nii.get_fdata()
    print("shape nii np array")
    print(mask.shape)
    mask_return = nib.Nifti1Image(mask, mask_nii.affine, mask_nii.header)
    print(mask_return.GetDirection())
    """
    ...
    #print("Image After postprocess 1")
    #print(image.shape)
    #Postprocessing
    if (image.sum() * (volume_per_voxel/1000)) < 4:
        image = np.zeros(image.shape)

    #print(np.unique(image))
    #print("Image After postprocess2")
    #print(image.shape)
    # print(input_file, "kept:", kept_size)
    img_out_itk = sitk.GetImageFromArray(image)
    img_out_itk = copy_geometry(img_out_itk, img_in)

    img_out_itk.CopyInformation(img_in)

    os.remove(output_file[0])
    sitk.WriteImage(img_out_itk, output_file[0])
